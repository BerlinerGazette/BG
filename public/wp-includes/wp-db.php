...
class wpdb {
	...
	function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'Ungültiges Datenbankpräfix'/*/WP_I18N_DB_BAD_PREFIX*/);
	...
	function select( $db, $dbh = null ) {
		...
		if ( !@mysql_select_db( $db, $dbh ) ) {
			$this->ready = false;
			$this->bail( sprintf( /*WP_I18N_DB_SELECT_DB*/'<h1>Die Datenbank kann nicht ausgewählt werden</h1>\\n
 ...
 <p>Wenn du nicht weißt, wie du die Datenbank einrichtest, <strong>kontaktiere deinen Provider</strong>. Wenn du gar nicht weiterkommst, findest Du im <a href=\\"http://wordpress.org/support/\\">englischsprachigen</a> und <a href=\\"http://forum.wpde.org/\\">deutschsprachigen Support-Forum</a> Hilfe.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser ), 'db_select_fail' );
	...
	function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR_FULL*/'WordPress-Datenbank-Fehler %1$s für Abfrage %2$s von %3$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller );
		else
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR*/'WordPress-Datenbank-Fehler %1$s für die Abfrage %2$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query );
